...
namespace protection {
    ...
    const wchar_t* get_string(int index) {
        ...
        switch (index) {
        case 0: value = xorstr("Qt5QWindowIcon"); break;
        case 1: value = xorstr("OLLYDBG"); break;
        case 2: value = xorstr("SunAwtFrame"); break;
        case 3: value = xorstr("ID"); break;
        case 4: value = xorstr("ntdll.dll"); break;
        case 5: value = xorstr("antidbg"); break;
 //       case 6: value = xorstr("%random_environment_var_name_that_doesnt_exist?[]<>@\\;*!-{}#:/~%"); break;
 //       case 7: value = xorstr("%random_file_name_that_doesnt_exist?[]<>@\\;*!-{}#:/~%"); break;
        }
    ...
    int memory::nt_query_information_process() {
        ...
        NtQueryInformationProcess = (_NtQueryInformationProcess)GetProcAddress(h_ntdll, xorstr("NtQueryInformationProcess"));
    ...
    int memory::debug_active_process() {
        ...
        snprintf(cmdline, sizeof(cmdline), xorstr("%ws %d"), sz_path, pid);
...
/*    int cpu::mov_ss() {
    ...
    int virtualization::check_registry() {
        HKEY h_key = 0;
        if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, xorstr("HARDWARE\\ACPI\\DSDT\\VBOX__"), 0, KEY_READ, &h_key) == ERROR_SUCCESS) { return debug_results::check_registry; }
